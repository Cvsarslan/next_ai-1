/* WhatsApp floating chat button — next_ai */
(function () {
    "use strict";
    ...
    var WA_SVG = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">'
        ...
        + '-1.3-2.5-5-3.9-10.5-6.6z"/></svg>';

    function mountButton(phone, greeting, agentName, agentStatus) {
        if (document.getElementById("wa-fab")) return;

        var waUrl = "https://wa.me/" + phone
            + "?text=" + encodeURIComponent(greeting || "Hello! I need assistance.");
        ...
        popup.innerHTML =
            ...
            + '<div class="wa-popup-header-icon">' + WA_SVG + '</div>'
            + '<div class="wa-popup-header-text">'
            +   '<h4>' + (agentName || "Support") + '</h4>'
            +   '<p>' + (agentStatus || "Typically replies within minutes") + '</p>'
            + '</div>'
            + '<button class="wa-popup-close" id="wa-close">&times;</button>'
            + '</div>'
            + '<div class="wa-popup-body">'
            +   '<div class="wa-popup-bubble">'
            +     '&#128075; Hi there! How can we help you today?<br>Click below to chat on WhatsApp.'
            +   '</div>'
            + '</div>'
            + '<div class="wa-popup-footer">'
            +   '<a class="wa-popup-start-btn" href="' + waUrl + '" target="_blank" rel="noopener">'
            +     WA_SVG + ' Start Chat'
            +   '</a>'
            + '</div>';
        ...
        fab.innerHTML =
            ...
            + '<button class="wa-fab-btn" id="wa-fab-btn" title="Chat on WhatsApp">'
            +   WA_SVG
            + '</button>';
        ...
        document.getElementById("wa-fab-btn").addEventListener("click", function (e) {
            e.stopPropagation();
            popup.classList.toggle("wa-open");
        });
    ...
    function tryMount() {
        ...
        /* 1. Try frappe.boot (populated by boot_session hook) */
        var b = (typeof frappe !== "undefined" && frappe.boot) ? frappe.boot : {};
        var phone   = b.wa_chat_phone || "";
        var enabled = (b.wa_chat_enabled === undefined || b.wa_chat_enabled === null)
                      ? true
                      : (b.wa_chat_enabled && b.wa_chat_enabled !== "0" && b.wa_chat_enabled !== 0);

        if (phone && enabled) {
            mountButton(phone, b.wa_chat_greeting, b.wa_chat_agent_name, b.wa_chat_agent_status);
            return;
        }

        /* 2. Fallback — fetch via Server Script (for sessions where boot hook didn't run) */
        if (typeof frappe !== "undefined" && frappe.call) {
            frappe.call({
                method: "get_whatsapp_config",
                callback: function (r) {
                    if (!r || !r.message) return;
                    var d = r.message;
                    var p = d.wa_chat_phone || "";
                    var en = String(d.wa_chat_enabled) !== "0" && p;
                    if (en) {
                        mountButton(p, d.wa_chat_greeting, d.wa_chat_agent_name, d.wa_chat_agent_status);
                    }
                }
            });
        }
    ...
    /* Also try on DOM ready in case app_ready already fired */
    $(function () {
